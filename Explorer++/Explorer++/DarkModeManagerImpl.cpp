...
DarkModeManagerImpl::DarkModeManagerImpl(EventWindow *eventWindow, const Config *config) :
	m_config(config)
{
	...
	m_connections.push_back(eventWindow->windowMessageSignal.AddObserver(
		std::bind_front(&DarkModeManagerImpl::OnEventWindowMessage, this)));

	m_connections.push_back(
		m_config->theme.addObserver(std::bind(&DarkModeManagerImpl::OnThemeUpdated, this)));
...
bool DarkModeManagerImpl::IsDarkModeSupported() const
{
	return m_darkModeSupported;
}

bool DarkModeManagerImpl::IsDarkModeEnabled() const
{
	return m_darkModeEnabled;
}

void DarkModeManagerImpl::OnEventWindowMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
void DarkModeManagerImpl::OnSettingChange(const wchar_t *systemParameter)
{
...
void DarkModeManagerImpl::OnThemeUpdated()
{
	UpdateAppDarkModeStatus();
}

void DarkModeManagerImpl::UpdateAppDarkModeStatus()
{
	bool enable = ShouldEnableDarkMode();

	if (!m_darkModeSupported || IsHighContrastEnabled() || enable == m_darkModeEnabled)
	{
		return;
	}
	...
	NotifyDarkModeStatusChanged(enable);
}

bool DarkModeManagerImpl::ShouldEnableDarkMode() const
{
	return m_config->theme == +Theme::Dark
		|| (m_config->theme == +Theme::System && !IsSystemAppModeLight());
}

void DarkModeManagerImpl::AllowDarkModeForApp(bool allow)
{
...
void DarkModeManagerImpl::AllowDarkModeForWindow(HWND hwnd, bool allow)
{
...
bool DarkModeManagerImpl::IsSystemAppModeLight() const
{
	bool lightMode = true;
...
void DarkModeManagerImpl::FlushMenuThemes()
{
...
void DarkModeManagerImpl::RefreshImmersiveColorPolicyState()
{
...
LONG DarkModeManagerImpl::DetourOpenNcThemeData()
{
...
LONG DarkModeManagerImpl::RestoreOpenNcThemeData()
{
	return DetourTransaction(
		[]
		{
...
HTHEME WINAPI DarkModeManagerImpl::DetouredOpenNcThemeData(HWND hwnd, LPCWSTR classList)
{
